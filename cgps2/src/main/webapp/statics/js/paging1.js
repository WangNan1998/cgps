;(function($){
	let methods = {
		pageInit: function(options){
			 ...
			 * @type {Obeject}
			 */
			let opts = $.extend({},$.fn.zPager.defaults,options);
		...
		pageData: function(_v, _current){
			let opts = _v.data("options");
			let t = opts.totalData, p = opts.pageData, ajaxOpts = null;
			if(opts.ajaxSetData&&(typeof(opts.ajaxSetData)==='boolean')){
				if(opts.url!=='' && typeof(opts.url)==='string'){
					...
					if(ajaxOpts.rows.length>0){
						let ishasDataRender = (opts.dataRender && typeof(opts.dataRender)==='function');
		...
		dataRender: function(_v, _data){
			...
			if(opts.htmlBox===''||(typeof(opts.htmlBox)!=='Obeject')){
				...
				if(!abx.hasClass('pagerHtmlWrap')){
					let d = '<div class="pagerHtmlWrap"></div>';
					_v.before(d);
				}
				_v.prev().html(cells);
			}else{
		...
		pageRender: function(_v, _current){
			 ...
			 * @type {Obeject}
			 */
			let o = _v.data("options");
			let _page = o.pageCount;

			let _middle = parseInt(o.pageStep/2);
			let _tep = _middle-2;
			let _html = '';
		...
		ajaxData: function(_url, _current){
			 ...
			 * @param  {Object} ){ let parms [ajax url,current page number]
			 * @return {[type]}            [obj total rows]
			 */
			let _total = $.fn.zPager.defaults.totalData;
			return (function(){
				let parms = {'total':_total,'rows':[]};
				$.ajax({
					url: _url,
					type: 'get',
					data: {"page":_current},
